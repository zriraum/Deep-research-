...
# Note: This is required for async operations in Jupyter environments
nest_asyncio.apply()
...
async def llm_call(state: MessagesState):
    ...
    # Option to combine with custom tools (currently disabled)
...
def tool_node(state: dict):
    ...
    2. Executes all tool calls concurrently using async operations
    3. Returns formatted tool results

    The async execution is particularly important for MCP as it involves
    inter-process communication with the MCP server subprocess.
    """
...
agent_builder.add_conditional_edges(
    ...
    should_continue,
    {
        "environment": "environment",  # Continue to tool execution
        "__end__": END,               # Provide final answer
    },
)
agent_builder.add_edge("environment", "llm_call")  # Loop back for more processing
